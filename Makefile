...
ifeq ($(LATEX),pdflatex)
	...
	BUILDCOMMANDUPRIGHT=rm -f upright.aux && $(LATEX) upright && bibtex upright && $(LATEX) upright && $(LATEX) upright
	...
	BUILDCOMMANDUPRIGHT=rm -f upright.aux && $(LATEX) upright && bibtex upright && $(LATEX) upright && $(LATEX) upright && dvips -z -o upright.ps upright && ps2pdf upright.ps && rm -f head.tmp body.tmp
	BUILDCOMMANDCOUNT=
endif
...
.PHONY : clean
clean:
	rm -f *~ *.aux *.log *.bbl *.blg *.dvi *.tmp *.out *.blg *.bbl $(OUTPUT) $(MAIN)$(MAINEXT) $(MAIN).ps $(MAIN)-prl.ps $(MAIN)-prl.pdf
